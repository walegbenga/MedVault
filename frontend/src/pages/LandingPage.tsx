    ...
    <div style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: '2rem', padding: '4rem 1.5rem' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', justifyContent: 'center', maxWidth: 760 }}>
        {[
          ...
          { icon: '📜', title: 'Live Smart Contract',    desc: 'Deploy MedVaultRegistry.sol to Base. Grant/revoke via real transactions.' },
          { icon: '🌐', title: 'IPFS Storage',           desc: 'Encrypted blobs on IPFS. Only the CID hash lives on-chain.' },
          { icon: '🔍', title: 'Immutable Audit',        desc: 'Every grant/revoke is a Base transaction — verifiable on BaseScan.' },
        ].map(f => (
      ...
      <Modal open={howOpen} onClose={() => setHowOpen(false)} title="🚀 How MedVault Works">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem', fontSize: '0.875rem', color: 'var(--text2)', lineHeight: 1.7 }}>
          {[
            { n: 1, title: 'Choose your role', desc: 'Select Patient (to manage records) or Grantee (to view records shared with you).' },
            { n: 2, title: 'Connect your wallet', desc: 'Connect any EVM wallet. Sign a free message to derive your AES-256-GCM encryption key via HKDF.' },
            { n: 3, title: 'Deploy your registry (patients)', desc: 'On first connect, the app deploys your personal MedVaultRegistry contract to Base. One-time ~$0.05–0.10 gas.' },
            { n: 4, title: 'Upload records', desc: 'Records are encrypted in your browser. Only the keccak256 hash and IPFS CID go on-chain via addRecord().' },
            { n: 5, title: 'Grant selective access', desc: 'The grantee signs a free message and sends you their signature. You call grantAccess() which encrypts your AES key specifically for them.' },
            { n: 6, title: 'Grantee views records', desc: 'The grantee enters your contract address, the app checks canAccess() on-chain, then decrypts records using the shared key.' },
          ].map(s => (
            <div key={s.n} style={{ display: 'flex', gap: '0.75rem' }}>
              <div style={{ width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.72rem', fontWeight: 700, flexShrink: 0, marginTop: 2, background: 'rgba(0,229,204,0.12)', border: '1px solid var(--teal)', color: 'var(--teal)' }}>{s.n}</div>
              <div>
