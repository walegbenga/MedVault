...
function AppInner() {
  ...
  // Persist recovered state across refreshes using localStorage
  const [recovered, setRecovered] = useState(() => {
    if (typeof window === 'undefined') return false
    const addr = localStorage.getItem('verihealth_last_address')
    if (!addr) return false
    return localStorage.getItem(RECOVERED_KEY(addr)) === '1'
  })
  ...
  // Track current address in localStorage for recovered state init
  useEffect(() => {
    if (address) {
      localStorage.setItem('verihealth_last_address', address.toLowerCase())
      // Check if this wallet has already recovered
      const alreadyRecovered = localStorage.getItem(RECOVERED_KEY(address)) === '1'
      setRecovered(alreadyRecovered)
    }
  }, [address])
  ...
  // Auto-recover from chain when needed
  useEffect(() => {
    if (
      !address        ||
      !isConnected    ||
    ...
    const run = async () => {
      setRecovering(true)
      try {
        toast('inf', 'Syncing your records from chain…')
        const result = await reg.recoverFromChain(savedContract as Address, false)
        if (result.records.length > 0) {
          toast('ok', `Recovered ${result.records.length} record(s)!`)
        }
        setRecovered(true)
      } catch (e) {
        console.error('Recovery failed:', e)
        toast('err', 'Recovery failed. Please try again.')
        // Mark as recovered even on failure to prevent infinite loop
  ...
  // Logo click — patients go to dashboard, others go to role select
  ...
  return (
    <div>
          ...
          {isConnected && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
              <select
                ...
                style={{
                  padding: '0.3rem 0.6rem', borderRadius: 9,
                  fontFamily: 'var(--mono)', fontSize: '0.7rem',
                  background: 'rgba(0,82,255,0.1)',
                  border: '1px solid rgba(0,82,255,0.25)',
                  color: '#6699ff', cursor: 'pointer',
                }}
              >
        ...
        <div style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
          <div style={{ width: '100%', maxWidth: 720 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
              {[
                {
                  role: 'patient' as Role, icon: '🏥',
                  label: "I'm a Patient", color: 'var(--teal)', hoverBorder: 'var(--teal)',
                  desc: 'Upload and manage my own health records. Deploy my personal registry. Grant access to doctors or insurers.',
                },
                {
                  role: 'grantee' as Role, icon: '👁',
                  label: "I'm a Grantee", color: '#6699ff', hoverBorder: '#6699ff',
                  desc: 'A patient has shared their records with my wallet. I want to view the records granted to me.',
                },
                {
                  role: 'delegate' as Role, icon: '🩺',
                  label: "I'm a Delegate", color: 'var(--green)', hoverBorder: 'var(--green)',
                  desc: 'A patient has approved me to upload records on their behalf. I have been added as a delegate.',
                },
              ].map(opt => (
                <button
                  key={opt.role!}
                ...
                >
                  ...
                  <div style={{ fontFamily: 'var(--font)', fontSize: '1rem', fontWeight: 700, marginBottom: '0.4rem', color: opt.color }}>
                    {opt.label}
                  </div>
      ...
      {/* ── Patient: compliance ── */}
      {isConnected && !wrongChain && role === 'patient' && !recovering &&
       reg.contractAddress && page === 'compliance' && (
        <Compliance reg={reg} onBack={() => setPage('dashboard')} />
      )}

      {/* ── Patient: dashboard ── */}
      {isConnected && !wrongChain && role === 'patient' && !recovering &&
       (reg.contractAddress || reg.deploying) && page === 'dashboard' && (
